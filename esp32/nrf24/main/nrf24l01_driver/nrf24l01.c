...
esp_err_t nrf24_init(void)
{
    ...
    spi_device_interface_config_t devcfg = {
        .clock_speed_hz = 1 * 1000 * 1000,      // ← reduced from 8MHz → more reliable on cheap modules
        .mode           = 0,
...
nrf24_write_reg(RF_CH,     115);           // ← Changed to 115 (better WiFi avoidance)

nrf24_write_reg(RF_SETUP,  0x27);          // ← 250 kbps, max power (0 dBm) - most stable in noisy env
...
nrf24_write_reg(FEATURE,   0x00);
    ...
    nrf24_write_reg(CONFIG, (1<<PWR_UP) | (1<<PRIM_RX) | (1<<EN_CRC) | (1<<CRCO));
    vTaskDelay(pdMS_TO_TICKS(1));
    esp_rom_delay_us(100);  // small delay after each write
    uint8_t config_verify = nrf24_read_reg(CONFIG);
    ESP_LOGI(TAG, "CONFIG verify after PWR_UP = 0x%02X (expect ~0x3E)", config_verify);

    ce_high();
    vTaskDelay(pdMS_TO_TICKS(5));

  // ← NEW: Verify detection by reading STATUS register
    uint8_t status = nrf24_read_reg(STATUS);
    ESP_LOGI(TAG, "STATUS after init = 0x%02X", status);

    if (status == 0x0E) {
        ESP_LOGI(TAG, "nRF24L01 DETECTED and responding OK! (STATUS = 0x0E)");
    } else if (status == 0xFF) {
        ESP_LOGE(TAG, "nRF24L01 NOT DETECTED! (STATUS = 0xFF → wiring/power/SPI dead?)");
        return ESP_ERR_NOT_FOUND;
    } else {
        ESP_LOGW(TAG, "nRF24L01 responding but unusual STATUS = 0x%02X (may be marginal)", status);
    }

    // Optional: Read CONFIG register to double-check (should be ~0x3E or 0x7E after our writes)
    uint8_t config = nrf24_read_reg(CONFIG);
    ESP_LOGI(TAG, "CONFIG register = 0x%02X", config);
...
esp_err_t nrf24_send(const uint8_t *data, uint8_t len)
{
    ...
    ce_high();
    esp_rom_delay_us(50);       // Safer CE pulse (>10µs)
    ce_low();
    ESP_LOGI(TAG, "CE pulsed");
